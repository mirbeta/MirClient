...
uses
  ...
  Grobal2, DXDraws, cliUtil, magiceff, WIL, ClFunc, GList, Actor, StallSystem;
    ...
    SM_WWJATTACK, SM_WSJATTACK, SM_WTJATTACK: begin
        ...
        if (m_nCurrentAction = SM_WWJATTACK) then begin
          ...
          if Effect <> nil then begin
            ...
            PlaySound(10510, m_nCurrX, m_nCurrY);
            g_PlayScene.m_EffectList.Add(Effect);
          end;
        end;

        if (m_nCurrentAction = SM_WSJATTACK) then begin //雷霆一击
          PlaySound(123, m_nCurrX, m_nCurrY);
        ...
        if (m_nCurrentAction = SM_WTJATTACK) then begin //劈星斩
          PlaySound(124, m_nCurrX, m_nCurrY);
    ...
    if (m_btEffect = 50) then begin
      ...
      if m_nCurrentFrame < MaxIdx then begin
        ...
        if not g_gcGeneral[6] then begin
          if m_nCboHumWinOffSet >= 56000 then
...
begin
  ...
  if m_boSSkill then begin
    ...
    if (frame = 8) and (m_boSmiteWideHitS2 > 0) then begin
      ...
      PlaySoundName('Wav\squarehit.wav');
    end;
    ...
    if (frame = 5) and (m_boDigFragment) then begin
      ...
      PlaySound(s_strike_stone, m_nCurrX, m_nCurrY);
...
begin
  ...
  if m_nCurrentAction <> 0 then begin
    ...
    if m_boMsgMuch then begin
      ...
      {end else if m_boMsgMuch then begin
        if m_boUseCboLib then begin
          if m_btIsHero = 1 then
            dwFrameTimetime := Round(m_dwFrameTime / 1.35)
          else
            dwFrameTimetime := Round(m_dwFrameTime / 1.40);
        end else
          dwFrameTimetime := Round(m_dwFrameTime / 1.4);}
    end else
      ...
      if m_boUseMagic then begin
          ...
          if sskill then begin
            with m_CurMagic do begin
              g_PlayScene.NewMagic(Self,
                ...
                Poison);
              if boFly then begin
                PlaySound(m_nMagicFireSound, m_nCurrX, m_nCurrY)
              end else begin
                //if m_CurMagic.EffectNumber <> 116 then
                PlayActorWav(m_nMagicExplosionSound, targx, targy, g_MySelf.m_nCurrX, g_MySelf.m_nCurrY);
              end;
            end;
            m_boNewMagic := False;
          end;
        ...
        if m_nCurEffFrame = m_nSpellFrame - 1 then begin
          if (m_CurMagic.ServerMagicCode > 0) and (not bss or m_boNewMagic) then begin
            with m_CurMagic do begin
              //DScreen.AddChatBoardString(inttostr(EffectNumber), clWhite, clBlue);
              g_PlayScene.NewMagic(Self,
                ServerMagicCode,
                EffectNumber,
                m_nCurrX,
...
begin
  ...
  if not m_boDeath and not m_boUseCboLib then begin
    ...
    if m_nState and $00040000 <> 0 then begin
      ...
      //Idx := 328 + (m_nGenAniCount mod 9);
      //d := g_WEffectImg.GetCachedImage(Idx, ax, ay);


      //Idx := 810 + (m_nGenAniCount mod 14);
      //d := g_WMagic7Images2.GetCachedImage(Idx, ax, ay);
      if d <> nil then
        DrawBlend(dsurface, ShiftX + ax, ShiftY + ay, d, 1);
    end;
  ...
  if m_boUseMagic and (m_CurMagic.EffectNumber > 0) then begin //sm_spell
    if m_nCurEffFrame in [0..m_nSpellFrame - 1] then begin
      if m_boUseCboLib then begin
        ...
        if (m_CurMagic.EffectNumber in [10, 29, 34, 48]) then begin
          if m_CurMagic.spelllv > MAXMAGICLV then
            GetEffectBase(m_CurMagic.EffectNumber - 1 + 500, 0, wimg, Idx)
          else
            GetEffectBase(m_CurMagic.EffectNumber - 1, 0, wimg, Idx);
        end else begin
          if (m_CurMagic.EffectNumber in [4, 8, 9, 11, 12, 15, 21]) then begin
