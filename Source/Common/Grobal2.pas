...
const
//引擎版权
  sWellCome                 : string = '〓〓〓〓欢迎使用Asp引擎系统软件〓〓〓〓';
  sProgram                  : string = '程序制作：';
  sOwen                     : string = 'ASP引擎';
  sUpdate                   : string = '更新日期：2020/11/03';
  sVersion                  : string = '程序版本：Ver 1.00 Build 2020/11/03';
  sWebSite                  : string = '程序网站：http://www.Aspm2.com';
  sBbsSite                  : string = '程序论坛：http://bbs.Aspm2.com';
...
{$ENDIF}
  ...
  SM_SENDTITLES = 1510;
  SM_Allshine = 1511;  //2020年10月4日 咸鱼
...
//----------------------------------------------------
  ...
  RM_SENDTITLES = 10180;
  RM_Allshine = 10181;   //2020年10月4日 咸鱼增加
...
{$IFEND}
  ...
  TUserItem = packed record //56
    ...
    Dura: Word;
    DuraMax: Word;
    btValue: array[0..25] of Byte; //22..25   空：19~21
  ...
  TMapItemshine = packed record
    Name: string[35];
    shine:Word;
  end;
  pTMapItemshine = ^TMapItemshine;
...
//人物数据类型结构
  ...
  TClientConf = record
    ...
    boRUNHUMAN: Boolean;   //穿人
    boRUNMON: Boolean;    //穿怪
    boRunNpc: Boolean;   //穿NPC
    boWarRunAll: Boolean;   //刀刀龙影剑法
    btDieColor: Integer;   //死亡颜色
    wSpellTime: Integer;
    wHitIime: Integer;
    wItemFlashTime: Integer;
    btItemSpeed: Integer;   //装备速度 小火炬为60
    boCanStartRun: Boolean; //True = 免助跑
    boParalyCanRun: Boolean; //麻痹可跑动
    boParalyCanWalk: Boolean;   //麻痹可走动
    boParalyCanHit: Boolean;      //麻痹可攻击
    boParalyCanSpell: Boolean;   //麻痹可施法
    ...
    //20200726开关集合
    Button_DetectBox: Boolean; //灵媒功能
    Button_Mission: Boolean; //任务按钮
    Button_Shop: Boolean; //商铺按钮
    Button_Friend: Boolean; //关系按钮
    Button_PlusAbil: Boolean; //属性按钮
    Button_baoguo: Boolean;    //包裹
    ShowWinExpMode: Boolean; //获取经验值显示模式
    IgnoreStruck: Boolean; //稳如泰山
...
//{$ENDIF}
  ...
  TScriptTimers = array[0..14] of TScriptTimer;
...
{$ENDIF}
  ...
  TRcHeader = packed record
    ...
    sFileName: string[050]; //登录器名称，显示在登录器上的标题// = '热血传奇登陆器';
  ...
  TMarketLoad = record
    UserItem: TUserItem; // DB 历厘侩
    Index: Integer; // DB 牢郸胶
    MarketType: Integer; // 盒府等 酒捞袍 辆幅
    SetType: Integer; // 悸飘 酒捞袍 辆幅
    SellCount: Integer;
    SellPrice: Integer; // 魄概 啊拜
    ItemName: string[30]; // 酒捞袍捞抚
    MarketName: string[30]; // 魄概磊疙
    SellWho: string[20]; // 魄概磊
    Selldate: string[10]; // 魄概朝楼(0312311210 = 2003-12-31 12:10 )
    SellState: Word; // 1 = 魄概吝 , 2 = 魄概肯丰
    IsOK: Integer; // 搬苞蔼
  end;
  ...
  TSearchSellItem = record
    MarketName: string[25]; // 辑滚捞抚_NPC  捞抚捞 荤侩凳
    who: string[25]; // 酒捞袍 魄概磊 八祸矫 荤侩 ,
    ItemName: string[25]; // 酒捞袍 捞抚 八祸矫 荤侩
    MakeIndex: Integer; // 酒捞袍狼 蜡聪农 锅龋
    ItemType: Integer; // 酒捞抛 辆幅 八祸矫 荤侩
    ItemSet: Integer; // 酒捞袍 悸飘 炼雀矫 荤侩
    sellindex: Integer; // 魄概 牢郸胶 酒捞袍 混锭 , 秒家 , 陛咀雀荐殿俊 荤侩
    CheckType: Integer; // DB 狼 眉农鸥涝
    IsOK: Integer; // 搬苞蔼
    UserMode: Integer; // 1= 酒捞袍 荤扁  , 2= 磊脚狼 酒捞袍 八祸
    pList: TList; // 困殴酒捞袍狼 府胶飘
  end;
