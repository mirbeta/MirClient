...
uses
  Windows, Messages, WinSock, SysUtils, Classes, SyncObjs;
...
type
  ...
  TSocketErrorEvent = procedure(Sender: TObject; Socket: TCustomWinSocket; ErrorEvent: TErrorEvent;
...
begin
  ...
  if ErrorCode <> 0 then
    raise ESocketError.CreateResFmt(@sWindowsSocketError, [SysErrorMessage(ErrorCode), ErrorCode, 'WSAStartup']);
end;
...
begin
  ...
  if ErrorCode <> 0 then
    raise ESocketError.CreateResFmt(@sWindowsSocketError, [SysErrorMessage(ErrorCode), ErrorCode, 'WSACleanup']);
end;
...
procedure TCustomWinSocket.AsyncInitSocket(const Name, Address, Service: string; Port: Word; QueueSize: Integer;
  Client: Boolean);
var
  ErrorCode: Integer;
begin
      ...
      lsIdle:
        begin
          ...
          begin
            ...
            FLookupHandle := WSAAsyncGetHostByName(Handle, CM_LOOKUPCOMPLETE, PAnsiChar(AnsiString(Name)), FGetHostData,
              MAXGETHOSTSTRUCT);
      ...
      lsLookupAddress:
        begin
          ...
          begin
            if FGetHostData = nil then
              FGetHostData := AllocMem(MAXGETHOSTSTRUCT);
            FLookupHandle  := WSAASyncGetServByName(Handle, CM_LOOKUPCOMPLETE, PAnsiChar(AnsiString(Service)), 'tcp',
              FGetHostData, MAXGETHOSTSTRUCT);
...
begin
  if (FSocket = INVALID_SOCKET) or (Socket <> FSocket) then
    Exit;
  Event(Self, seRead);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Disconnect(FSocket);
      if ErrorCode <> 0 then
        raise ESocketError.CreateResFmt(@sWindowsSocketError, [SysErrorMessage(Message.AsyncError), Message.AsyncError,
          'ASync Lookup']);
      Exit;
    end;
    ...
    else if FLookupState = lsLookupService then
    begin
...
begin
  case SocketEvent of
    seAccept, seLookup, seConnecting, seListen:
      begin
      end;
    seConnect:
...
procedure TServerWinSocket.ClientError(Sender: TObject; Socket: TCustomWinSocket; ErrorEvent: TErrorEvent;
...
begin
  ...
  if getsockopt(INVALID_SOCKET, SOL_SOCKET, SO_OPENTYPE, PAnsiChar(@OldOpenType), Len) = 0 then
    try
...
begin
  ...
  begin
    if Assigned(FOnGetThread) then
      FOnGetThread(Self, ClientSocket, Result);
    if Result = nil then
      Result := DoCreateThread(ClientSocket);
  end;
end;
...
begin
  if Assigned(FOnClientError) then
    FOnClientError(Self, Socket, ErrorEvent, ErrorCode);
end;
...
procedure TServerClientThread.HandleError(Sender: TObject; Socket: TCustomWinSocket; ErrorEvent: TErrorEvent;
  var ErrorCode: Integer);
begin
...
procedure TAbstractSocket.DoError(Sender: TObject; Socket: TCustomWinSocket; ErrorEvent: TErrorEvent;
  var ErrorCode: Integer);
begin
...
begin
  ...
  try
    ...
    if not ReadFile(FSocket.SocketHandle, Buffer, Count, DWORD(Result), @Overlapped) and
...
begin
  ...
  try
    FillChar(Overlapped, SizeOf(Overlapped), 0);
    Overlapped.hEvent := FEvent.Handle;
    if not WriteFile(FSocket.SocketHandle, Buffer, Count, DWORD(Result), @Overlapped) and
      (GetLastError <> ERROR_IO_PENDING) then
    begin
      ErrorCode := GetLastError;
...
begin
  ...
  begin
    if FServerSocket.Connected then
      ...
      FServerSocket.Listen(FHost, FAddress, FService, FPort, SOMAXCONN);
  end;
end;
