...
uses
  ...
  Classes, Dialogs, ComCtrls, ImgList, ExtDlgs, uDesignEditors, uDesignIntf;
...
type
  ...
  TuImageListEditor = class(TComponentEditor)
...
uses uToolsAPI, CommCtrl, TypInfo, uDsnConst;
...
begin
  if (AImageList.Width > 256) or (AImageList.Height > 256) then
    raise ENonAIRException.Create('图片长宽不可超过256');
        ...
        begin
          ...
          begin
            ...
            if ((XDiv > 1) or (YDiv > 1)) and
              ...
              DialogResult := MessageDlg(Format('是否将图片"%s"分割成%d张独立的小图片?',
...
procedure TuImageListEditor.Edit;
var
  _Self: TImageList;
  TempImageList: TImageList;
begin
  _Self :=  TImageList(GetComponent);
  EditImageList(_Self);
end;

procedure TuImageListEditor.ExecuteVerb(Index: Integer);
begin
  case Index of
    0:  Edit;
  end;
...
function TuImageListEditor.GetVerb(Index: Integer): string;
begin
  case Index of
    0:  Result  :=  '编辑';
  end;
...
function TuImageListEditor.GetVerbCount: Integer;
begin
  Result  :=  1;
end;

initialization
  uDesignIntf.RegisterComponentEditor(TCustomImageList, TuImageListEditor);
