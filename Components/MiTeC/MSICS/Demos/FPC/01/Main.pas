...
uses
  Windows, Messages, SysUtils, Graphics, Controls, Forms, Dialogs, StdCtrls, ExtCtrls, ComCtrls,
  MSI_CPU, MSI_SystemInfo;
...
uses
  MiTeC_USB, MiTeC_Routines;

{$R *.lfm}

procedure TForm1.FormCreate(Sender: TObject);
begin
  SI:=TMiTeC_SystemInfo.Create(Self);
  Caption:=SI._About;
end;
...
begin
  ...
  try
    ...
    if SI.Media.SoundCardIndex>-1 then
      n:=Tree.Items.AddChild(r,SI.Devices.Devices[SI.Media.SoundCardIndex].Name)
    else
      with SI.Media do
