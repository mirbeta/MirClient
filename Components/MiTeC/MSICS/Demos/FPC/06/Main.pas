...
uses ShellAPI, MiTeC_CtrlRtns, MiTeC_Datetime, MiTeC_Dialogs, PrcDetails,
  MiTeC_Routines, MiTeC_AdvAPI, SvcDetails, SMDetails, JwaWinSvc, MSI_Defs;
...
begin
  ...
  if (r.Status in [SERVICE_RUNNING,SERVICE_PAUSED]) and YesNo(Format('Stop %s service?',[r.DisplayName])) then begin
    if ServiceStop(r.Name) then
      FSLM.Suspended:=False
    else
