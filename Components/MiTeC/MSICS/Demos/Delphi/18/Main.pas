...
uses
  ...
  Dialogs, ComCtrls, ExtCtrls, Gauges, StdCtrls, MiTeC_NativeDefs, MSI_ProcListMon, MSI_ProcMon;
...
type
  TwndMain = class(TForm)
    ...
    procedure lPubClick(Sender: TObject);
  private
...
uses TlHelp32, MiTeC_StrUtils, MiTeC_Routines, MiTeC_NativeAPI, MiTeC_Datetime,
  Math, MiTeC_Cert, MiTeC_WinTrust, MiTeC_Windows;
...
procedure TwndMain.FindProcess;
var
  n: TListItem;
begin
  if Trim(AText)='' then
...
procedure TwndMain.lPubClick(Sender: TObject);
begin
  if TLabel(Sender).Font.Style<>[] then
    VerifyFile(FPM.ImageName,Handle);
end;
...
begin
  ...
  try
    ...
    for i:=0 to c-1 do begin
      ...
      with HandleList.Items.Add do begin
        ...
        SubItems.Add(Format('%s',[h.TypeName]));
        Subitems.Add(h.Name);
      end;
    end;
  finally
  ...
  if HandleList.Items.Count>0 then begin
    ...
    try
      ...
      HandleList.Scroll(0,vp*(r.Bottom-r.Top));
      if hid<>0 then
...
begin
  ...
  if ModList.Items.Count>0 then begin
    ...
    try
      ...
      ModList.Scroll(0,vp*(r.Bottom-r.Top));
      if mid<>'' then
...
begin
  ...
  if ThreadList.Items.Count>0 then begin
    ...
    try
      ...
      ThreadList.Scroll(0,vp*(r.Bottom-r.Top));
      if tid<>-1 then
