...
uses ShellAPI, MSI_Defs, MiTeC_Routines, MSI_SMBIOS, ZLIB, MiTeC_WinCrypt, ComObj, ActiveX, MiTeC_SS, MiTeC_Windows;
...
begin
  ...
  try
    ...
    lRes.Caption:=Format('Elapsed time: %1.2f s (%1.0n B)',[(GetTickCount-t)/1000,GetFileSize(fn)/1]);
    bLoad.Enabled:=True;
  finally
...
begin
  ...
  try
    ...
    lRes.Caption:=Format('Elapsed time: %1.2f s',[(GetTickCount-t)/1000]);
    FillList;
    bSave.Enabled:=False;
  finally
    Screen.Cursor:=crDefault;
