{*******************************************************}
{                                                       }
{         Windows Cryptography API interface            }
{                                                       }
...
type
  ...
  CRYPTOAPI_BLOB = _CRYPTOAPI_BLOB;
...
const
  AES_BlockSize = 16; //128bits

type
  TAES_128_BLOB = packed record
    Header: TPublicKeyStruc;
    KeySize: LongWord;
    Key: array [0..15] of Byte;
  end;

  TAES_192_BLOB = packed record
    Header: TPublicKeyStruc;
    KeySize: LongWord;
    Key: array [0..23] of Byte;
  end;

  TAES_256_BLOB = packed record
    Header: TPublicKeyStruc;
    KeySize: LongWord;
    Key: array [0..31] of Byte;
  end;

  TAES_IV = packed array [0..AES_BlockSize-1] of Byte;
...
function CryptStringToBinaryA(pszString: LPCSTR; cchString: DWORD;
  ...
  ppdwSkip,pdwFlags: PDWORD): BOOL; stdcall;
...
{$IFDEF UNICODE}
function CryptStringToBinary(pszString: LPCWSTR; cchString: DWORD;
  dwFlags: DWORD; ppBinary: PBYTE; var ppcbBinary: DWORD;
  ppdwSkip,pdwFlags: PDWORD): BOOL; stdcall;
{$ELSE}
...
function PFXImportCertStore(pPFX: PCRYPT_DATA_BLOB; szPassword: LPCWSTR; dwFlags: DWORD): HCERTSTORE; stdcall;
...
function CreateTextHash(AData: string; AHashType: Cardinal = CALG_SHA1): TBytes;
function CreateFileHash(AFilename: string; AHashType: Cardinal = CALG_SHA1; ABufferSize: Cardinal = 1024): TBytes;
function CreateStreamHash(AStream: TStream; AHashType: Cardinal = CALG_SHA1; ABufferSize: Cardinal = 1024): TBytes;
function CreateBufferHash(ABuffer: PByte; ASize: int64; AHashType: Cardinal = CALG_SHA1; ABufferSize: Cardinal = 1024): TBytes;

procedure CryptStream(const ASource, ADest: TStream; APassword: string; AToCrypt: Boolean; AHashType: Cardinal = CALG_SHA1; ACipherType: Cardinal = CALG_RC4); overload;
function CryptBuffer(ABuffer: PByte; ASize: int64; APassword: string; AToCrypt: Boolean; AHashType: Cardinal = CALG_SHA1; ACipherType: Cardinal = CALG_RC4): TBytes;
function CryptBytes(const ASource: TBytes; APassword: string; AToCrypt: Boolean; AHashType: Cardinal = CALG_SHA1; ACipherType: Cardinal = CALG_RC4): TBytes; overload;
procedure CryptFile(const ASource, ADest: string; APassword: string; AToCrypt: Boolean; AHashType: Cardinal = CALG_SHA1; ACipherType: Cardinal = CALG_RC4);
function EncryptText(const ASource, APassword: string; AHashType: Cardinal = CALG_SHA1; ACipherType: Cardinal = CALG_RC4): TBytes;

procedure CryptAESStream(const ASource, ADest: TStream; APassword, AInitVector: string; AEncrypt: Boolean; ACipherMode: Cardinal = CRYPT_MODE_CBC; AHashType: Cardinal = CALG_SHA_256; ACipherType: Cardinal = CALG_AES_128);
function CryptAESBytes(const ASource: TBytes; const APassword, AInitVector: string; AEncrypt: Boolean; ACipherMode: Cardinal = CRYPT_MODE_CBC; AHashType: Cardinal = CALG_SHA_256; ACipherType: Cardinal = CALG_AES_128): TBytes;
function EncryptAESText(const ASource, APassword, AInitVector: string; ACipherMode: Cardinal = CRYPT_MODE_CBC; AHashType: Cardinal = CALG_SHA_256; ACipherType: Cardinal = CALG_AES_128): TBytes;

function MakeKeys(var APrivate, APublic: TBytes): Boolean;
procedure CryptStream(const ASource, ADest: TStream; AKey: TBytes; AToCrypt: Boolean; ACipherType: Cardinal = CALG_RC4); overload;
function CryptBytes(const ASource: TBytes; AKey: TBytes; AToCrypt: Boolean; ACipherType: Cardinal = CALG_RC4): TBytes; overload;

function FormatToBinary(ASource: string; var ASize: Cardinal; AFormat: Cardinal = CRYPT_STRING_HEX): PByte;
function BinaryToFormat(ASource: PByte; ASize: Cardinal; AFormat: Cardinal = CRYPT_STRING_HEX): string;
...
type
  TCryptAcquireContextA = function(var phProv: HCRYPTPROV; pszContainer: LPCSTR;
  pszProvider: LPCSTR; dwProvType: DWORD; dwFlags: DWORD): BOOL; stdcall;
var
  _CryptAcquireContextA: TCryptAcquireContextA = nil;

function CryptAcquireContextA;
begin
  if not Assigned(_CryptAcquireContextA) then
    GetProcedureAddress(Pointer(@_CryptAcquireContextA), advapi32, 'CryptAcquireContextA');
  if Assigned(_CryptAcquireContextA) then
    Result:=_CryptAcquireContextA(phProv,pszContainer,pszProvider,dwProvType,dwFlags)
  else
    Result:=False;
end;

type
  TCryptAcquireContextW = function(var phProv: HCRYPTPROV; pszContainer: LPCWSTR;
  pszProvider: LPCWSTR; dwProvType: DWORD; dwFlags: DWORD): BOOL; stdcall;
var
...
begin
  if not Assigned(_CryptAcquireContextW) then
    GetProcedureAddress(Pointer(@_CryptAcquireContextW), advapi32, 'CryptAcquireContextW');
  if Assigned(_CryptAcquireContextW) then
    Result:=_CryptAcquireContextW(phProv,pszContainer,pszProvider,dwProvType,dwFlags)
  else
    Result:=False;
end;
...
begin
  {$IFDEF UNICODE}
  Result:=CryptAcquireContextW(phProv,pszContainer,pszProvider,dwProvType,dwFlags);
  {$ELSE}
  Result:=CryptAcquireContextA(phProv,pszContainer,pszProvider,dwProvType,dwFlags);
  {$ENDIF}
end;

type
  TCryptReleaseContext = function(hProv: HCRYPTPROV; dwFlags: ULONG_PTR): BOOL; stdcall;
var
  _CryptReleaseContext: TCryptReleaseContext = nil;

function CryptReleaseContext;
begin
  if not Assigned(_CryptReleaseContext) then
    GetProcedureAddress(Pointer(@_CryptReleaseContext), advapi32, 'CryptReleaseContext');
  if Assigned(_CryptReleaseContext) then
    Result:=_CryptReleaseContext(hProv,dwFlags)
  else
    Result:=False;
end;

type
 TCryptGenKey = function(hProv: HCRYPTPROV; Algid: ALG_ID; dwFlags: DWORD;
  var phKey: HCRYPTKEY): BOOL; stdcall;
var
  _CryptGenKey: TCryptGenKey = nil;

function CryptGenKey;
begin
  if not Assigned(_CryptGenKey) then
    GetProcedureAddress(Pointer(@_CryptGenKey), advapi32, 'CryptGenKey');
  if Assigned(_CryptGenKey) then
    Result:=_CryptGenKey(hProv,Algid,dwFlags,phKey)
  else
    Result:=False;
end;

type
  TCryptDeriveKey = function(hProv: HCRYPTPROV; Algid: ALG_ID; hBaseData: HCRYPTHASH;
  dwFlags: DWORD; var phKey: HCRYPTKEY): BOOL; stdcall;
var
  _CryptDeriveKey: TCryptDeriveKey = nil;

function CryptDeriveKey;
begin
  if not Assigned(_CryptDeriveKey) then
    GetProcedureAddress(Pointer(@_CryptDeriveKey), advapi32, 'CryptDeriveKey');
  if Assigned(_CryptDeriveKey) then
    Result:=_CryptDeriveKey(hProv,Algid,hBaseData,dwFlags,phKey)
  else
    Result:=False;
end;
...
var
  _CryptDestroyKey: TCryptDestroyKey = nil;

function CryptDestroyKey;
begin
  if not Assigned(_CryptDestroyKey) then
    GetProcedureAddress(Pointer(@_CryptDestroyKey), advapi32, 'CryptDestroyKey');
  if Assigned(_CryptDestroyKey) then
    Result:=_CryptDestroyKey(hKey)
  else
    Result:=False;
end;

type
  TCryptSetKeyParam = function(hKey: HCRYPTKEY; dwParam: DWORD; pbData: LPBYTE;
  dwFlags: DWORD): BOOL; stdcall;
var
  _CryptSetKeyParam: TCryptSetKeyParam = nil;

function CryptSetKeyParam;
begin
  if not Assigned(_CryptSetKeyParam) then
    GetProcedureAddress(Pointer(@_CryptSetKeyParam), advapi32, 'CryptSetKeyParam');
  if Assigned(_CryptSetKeyParam) then
    Result:=_CryptSetKeyParam(hKey,dwParam,pbData,dwFlags)
  else
    Result:=False;
end;

type
  TCryptGetKeyParam = function(hKey: HCRYPTKEY; dwParam: DWORD; pbData: LPBYTE;
  var pdwDataLen: DWORD; dwFlags: DWORD): BOOL; stdcall;
var
  _CryptGetKeyParam: TCryptGetKeyParam = nil;

function CryptGetKeyParam;
begin
  if not Assigned(_CryptGetKeyParam) then
    GetProcedureAddress(Pointer(@_CryptGetKeyParam), advapi32, 'CryptGetKeyParam');
  if Assigned(_CryptGetKeyParam) then
    Result:=_CryptGetKeyParam(hKey,dwParam,pbData,pdwDataLen,dwFlags)
  else
    Result:=False;
end;

type
  TCryptSetHashParam = function(hHash: HCRYPTHASH; dwParam: DWORD; pbData: LPBYTE;
  dwFlags: DWORD): BOOL; stdcall;
var
  _CryptSetHashParam: TCryptSetHashParam = nil;

function CryptSetHashParam;
begin
  if not Assigned(_CryptSetHashParam) then
    GetProcedureAddress(Pointer(@_CryptSetHashParam), advapi32, 'CryptSetHashParam');
  if Assigned(_CryptSetHashParam) then
    Result:=_CryptSetHashParam(hHash,dwParam,pbData,dwFlags)
  else
    Result:=False;
end;

type
  TCryptGetHashParam = function(hHash: HCRYPTHASH; dwParam: DWORD; pbData: LPBYTE;
  var pdwDataLen: DWORD; dwFlags: DWORD): BOOL; stdcall;
var
  _CryptGetHashParam: TCryptGetHashParam = nil;

function CryptGetHashParam;
begin
  if not Assigned(_CryptGetHashParam) then
    GetProcedureAddress(Pointer(@_CryptGetHashParam), advapi32, 'CryptGetHashParam');
  if Assigned(_CryptGetHashParam) then
    Result:=_CryptGetHashParam(hHash,dwParam,pbData,pdwDataLen,dwFlags)
  else
    Result:=False;
end;

type
  TCryptSetProvParam = function(hProv: HCRYPTPROV; dwParam: DWORD; pbData: LPBYTE;
  dwFlags: DWORD): BOOL; stdcall;
var
  _CryptSetProvParam: TCryptSetProvParam = nil;

function CryptSetProvParam;
begin
  if not Assigned(_CryptSetProvParam) then
    GetProcedureAddress(Pointer(@_CryptSetProvParam), advapi32, 'CryptSetProvParam');
  if Assigned(_CryptSetProvParam) then
    Result:=_CryptSetProvParam(hProv,dwParam,pbData,dwFlags)
  else
    Result:=False;
end;

type
  TCryptGetProvParam = function(hProv: HCRYPTPROV; dwParam: DWORD; pbData: LPBYTE;
  var pdwDataLen: DWORD; dwFlags: DWORD): BOOL; stdcall;
var
...
begin
  if not Assigned(_CryptGetProvParam) then
    GetProcedureAddress(Pointer(@_CryptGetProvParam), advapi32, 'CryptGetProvParam');
  if Assigned(_CryptGetProvParam) then
    Result:=_CryptGetProvParam(hProv,dwParam,pbData,pdwDataLen,dwFlags)
  else
    Result:=False;
end;

type
  TCryptGenRandom = function(hProv: HCRYPTPROV; dwLen: DWORD; pbBuffer: LPBYTE): BOOL; stdcall;
var
  _CryptGenRandom: TCryptGenRandom = nil;

function CryptGenRandom;
begin
  if not Assigned(_CryptGenRandom) then
    GetProcedureAddress(Pointer(@_CryptGenRandom), advapi32, 'CryptGenRandom');
  if Assigned(_CryptGenRandom) then
    Result:=_CryptGenRandom(hProv,dwLen,pbBuffer)
  else
    Result:=False;
end;

type
  TCryptGetUserKey = function(hProv: HCRYPTPROV; dwKeySpec: DWORD; var phUserKey: HCRYPTKEY): BOOL; stdcall;
var
  _CryptGetUserKey: TCryptGetUserKey = nil;

function CryptGetUserKey;
begin
  if not Assigned(_CryptGetUserKey) then
    GetProcedureAddress(Pointer(@_CryptGetUserKey), advapi32, 'CryptGetUserKey');
  if Assigned(_CryptGetUserKey) then
    Result:=_CryptGetUserKey(hProv,dwKeySpec,phUserKey)
  else
    Result:=False;
end;

type
  TCryptExportKey = function(hKey, hExpKey: HCRYPTKEY; dwBlobType, dwFlags: DWORD;
  pbData: LPBYTE; var pdwDataLen: DWORD): BOOL; stdcall;
var
  _CryptExportKey: TCryptExportKey = nil;

function CryptExportKey;
begin
  if not Assigned(_CryptExportKey) then
    GetProcedureAddress(Pointer(@_CryptExportKey), advapi32, 'CryptExportKey');
  if Assigned(_CryptExportKey) then
    Result:=_CryptExportKey(hKey,hExpKey,dwBlobType,dwFlags,pbData,pdwDataLen)
  else
    Result:=False;
end;

type
  TCryptImportKey = function(hProv: HCRYPTPROV; pbData: LPBYTE; dwDataLen: DWORD;
  hPubKey: HCRYPTKEY; dwFlags: DWORD; var phKey: HCRYPTKEY): BOOL; stdcall;
var
  _CryptImportKey: TCryptImportKey = nil;

function CryptImportKey;
begin
  if not Assigned(_CryptImportKey) then
    GetProcedureAddress(Pointer(@_CryptImportKey), advapi32, 'CryptImportKey');
  if Assigned(_CryptImportKey) then
    Result:=_CryptImportKey(hProv,pbData,dwDataLen,hPubKey,dwFlags,phKey)
  else
    Result:=False;
end;

type
  TCryptEncrypt = function(hKey: HCRYPTKEY; hHash: HCRYPTHASH; Final: BOOL;
  dwFlags: DWORD; pbData: LPBYTE; var pdwDataLen: DWORD; dwBufLen: DWORD): BOOL; stdcall;
var
  _CryptEncrypt: TCryptEncrypt = nil;

function CryptEncrypt;
begin
  if not Assigned(_CryptEncrypt) then
    GetProcedureAddress(Pointer(@_CryptEncrypt), advapi32, 'CryptEncrypt');
  if Assigned(_CryptEncrypt) then
    Result:=_CryptEncrypt(hKey,hHash,Final,dwFlags,pbData,pdwDataLen,dwBufLen)
  else
    Result:=False;
end;

type
  TCryptDecrypt = function(hKey: HCRYPTKEY; hHash: HCRYPTHASH; Final: BOOL;
  dwFlags: DWORD; pbData: LPBYTE; var pdwDataLen: DWORD): BOOL; stdcall;
var
  _CryptDecrypt: TCryptDecrypt = nil;

function CryptDecrypt;
begin
  if not Assigned(_CryptDecrypt) then
    GetProcedureAddress(Pointer(@_CryptDecrypt), advapi32, 'CryptDecrypt');
  if Assigned(_CryptDecrypt) then
    Result:=_CryptDecrypt(hKey,hHash,Final,dwFlags,pbData,pdwDataLen)
  else
    Result:=False;
end;

type
  TCryptCreateHash = function(hProv: HCRYPTPROV; Algid: ALG_ID; hKey: HCRYPTKEY;
  dwFlags: DWORD; var phHash: HCRYPTHASH): BOOL; stdcall;
var
  _CryptCreateHash: TCryptCreateHash = nil;

function CryptCreateHash;
begin
  if not Assigned(_CryptCreateHash) then
    GetProcedureAddress(Pointer(@_CryptCreateHash), advapi32, 'CryptCreateHash');
  if Assigned(_CryptCreateHash) then
    Result:=_CryptCreateHash(hProv,Algid,hKey,dwFlags,phHash)
  else
    Result:=False;
end;

type
  TCryptHashData = function(hHash: HCRYPTHASH; pbData: LPBYTE; dwDataLen, dwFlags: DWORD): BOOL; stdcall;
var
  _CryptHashData: TCryptHashData = nil;

function CryptHashData;
begin
  if not Assigned(_CryptHashData) then
    GetProcedureAddress(Pointer(@_CryptHashData), advapi32, 'CryptHashData');
  if Assigned(_CryptHashData) then
    Result:=_CryptHashData(hHash,pbData,dwDataLen,dwFlags)
  else
    Result:=False;
end;

type
  TCryptHashSessionKey = function(hHash: HCRYPTHASH; hKey: HCRYPTKEY; dwFlags: DWORD): BOOL; stdcall;
var
  _CryptHashSessionKey: TCryptHashSessionKey = nil;

function CryptHashSessionKey;
begin
  if not Assigned(_CryptHashSessionKey) then
    GetProcedureAddress(Pointer(@_CryptHashSessionKey), advapi32, 'CryptHashSessionKey');
  if Assigned(_CryptHashSessionKey) then
    Result:=_CryptHashSessionKey(hHash,hKey,dwFlags)
  else
    Result:=False;
end;
...
var
  _CryptDestroyHash: TCryptDestroyHash = nil;

function CryptDestroyHash;
begin
  if not Assigned(_CryptDestroyHash) then
    GetProcedureAddress(Pointer(@_CryptDestroyHash), advapi32, 'CryptDestroyHash');
  if Assigned(_CryptDestroyHash) then
    Result:=_CryptDestroyHash(hHash)
  else
    Result:=False;
end;
...
begin
  GetProcedureAddress(_CryptSignHash, advapi32, 'CryptSignHash' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF});
  asm
        MOV     ESP, EBP
        POP     EBP
        JMP     [_CryptSignHash]
...
begin
  GetProcedureAddress(_CryptVerifySignature, advapi32, 'CryptVerifySignature' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF});
  asm
        MOV     ESP, EBP
        POP     EBP
...
begin
  GetProcedureAddress(_CryptSetProvider, advapi32, 'CryptSetProvider' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF});
  asm
        MOV     ESP, EBP
        POP     EBP
...
begin
  GetProcedureAddress(_CryptSetProviderEx, advapi32, 'CryptSetProviderEx' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF});
  asm
        MOV     ESP, EBP
        POP     EBP
...
begin
  GetProcedureAddress(_CryptGetDefaultProvider, advapi32, 'CryptGetDefaultProvider' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF});
  asm
        MOV     ESP, EBP
        POP     EBP
...
begin
  GetProcedureAddress(_CryptEnumProviderTypes, advapi32, 'CryptEnumProviderTypes' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF});
  asm
        MOV     ESP, EBP
        POP     EBP
...
begin
  GetProcedureAddress(_CryptEnumProviders, advapi32, 'CryptEnumProviders' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF});
  asm
        MOV     ESP, EBP
        POP     EBP
...
type
  TCryptDecodeObjectEx = function(dwCertEncodingType: DWORD; lpszStructType: LPCSTR;
                                  pbEncoded: Pointer; cbEncoded, dwFlags: DWORD; pDecodePara: PCRYPT_DECODE_PARA;
                                  pvStructInfo: Pointer; var pcbStructInfo: DWORD): BOOL; stdcall;
var
  _CryptDecodeObjectEx: TCryptDecodeObjectEx = nil;

function CryptDecodeObjectEx;
begin
  if not Assigned(_CryptDecodeObjectEx) then
    GetProcedureAddress(Pointer(@_CryptDecodeObjectEx), crypt32, 'CryptDecodeObjectEx');
  if Assigned(_CryptDecodeObjectEx) then
    Result:=_CryptDecodeObjectEx(dwCertEncodingType,lpszStructType,pbEncoded,cbEncoded,dwFlags,
...
type
  TCryptDecodeObject = function(dwCertEncodingType: DWORD; lpszStructType: LPCSTR;
                                pbEncoded: LPBYTE; cbEncoded, dwFlags: DWORD; pvStructInfo: Pointer;
                                var pcbStructInfo: DWORD): BOOL; stdcall;
var
  _CryptDecodeObject: TCryptDecodeObject = nil;

function CryptDecodeObject;
begin
  if not Assigned(_CryptDecodeObject) then
    GetProcedureAddress(Pointer(@_CryptDecodeObject), crypt32, 'CryptDecodeObject');
  if Assigned(_CryptDecodeObject) then
    Result:=_CryptDecodeObject(dwCertEncodingType,lpszStructType,pbEncoded,cbEncoded,dwFlags,pvStructInfo,pcbStructInfo)
  else
    Result:=False;
end;
...
type
  TCryptFindOIDInfo = function(dwKeyType: DWORD; pvKey: Pointer; dwGroupId: DWORD): PCCRYPT_OID_INFO; stdcall;
var
  _CryptFindOIDInfo: TCryptFindOIDInfo = nil;

function CryptFindOIDInfo;
begin
  if not Assigned(_CryptFindOIDInfo) then
    GetProcedureAddress(Pointer(@_CryptFindOIDInfo), crypt32, 'CryptFindOIDInfo');
  if Assigned(_CryptFindOIDInfo) then
    Result:=_CryptFindOIDInfo(dwKeyType,pvKey,dwGroupId)
  else
    Result:=nil;
end;
...
type
  TCertCreateCertificateContext = function(dwCertEncodingType: DWORD; pbCertEncoded: LPBYTE; cbCertEncoded: DWORD): PCCERT_CONTEXT; stdcall;
var
  _CertCreateCertificateContext: TCertCreateCertificateContext = nil;

function CertCreateCertificateContext;
begin
  if not Assigned(_CertCreateCertificateContext) then
    GetProcedureAddress(Pointer(@_CertCreateCertificateContext), crypt32, 'CertCreateCertificateContext');
  if Assigned(_CertCreateCertificateContext) then
    Result:=_CertCreateCertificateContext(dwCertEncodingType,pbCertEncoded,cbCertEncoded)
  else
    Result:=nil;
end;
...
begin
  if not Assigned(_CertCloseStore) then
    GetProcedureAddress(Pointer(@_CertCloseStore), crypt32, 'CertCloseStore');
  if Assigned(_CertCloseStore) then
    Result:=_CertCloseStore(hCertStore,dwFlags)
  else
    Result:=False;
end;
...
type
  TCertEnumCertificatesInStore = function(hCertStore: HCERTSTORE; pPrevCertContext: PCCERT_CONTEXT): PCCERT_CONTEXT; stdcall;
var
  _CertEnumCertificatesInStore: TCertEnumCertificatesInStore = nil;

function CertEnumCertificatesInStore;
begin
  if not Assigned(_CertEnumCertificatesInStore) then
    GetProcedureAddress(Pointer(@_CertEnumCertificatesInStore), crypt32, 'CertEnumCertificatesInStore');
  if Assigned(_CertEnumCertificatesInStore) then
    Result:=_CertEnumCertificatesInStore(hCertStore,pPrevCertContext)
  else
    Result:=nil;
end;
...
type
  TCertFindCertificateInStore = function(hCertStore: HCERTSTORE;
                                         ...
                                         pPrevCertContext: PCCERT_CONTEXT): PCCERT_CONTEXT; stdcall;
var
  _CertFindCertificateInStore: TCertFindCertificateInStore = nil;

function CertFindCertificateInStore;
begin
  if not Assigned(_CertFindCertificateInStore) then
    GetProcedureAddress(Pointer(@_CertFindCertificateInStore), crypt32, 'CertFindCertificateInStore');
  if Assigned(_CertFindCertificateInStore) then
    Result:=_CertFindCertificateInStore(hCertStore,dwCertEncodingType,dwFindFlags,dwFindType,pvFindPara,pPrevCertContext)
  else
    Result:=nil;
end;
...
type
  TCertOpenStore = function(lpszStoreProvider: LPCSTR; dwEncodingType: DWORD; hCryptProv: HCRYPTPROV; dwFlags: DWORD; pvPara: Pointer): HCERTSTORE; stdcall;
var
...
begin
  if not Assigned(_CertOpenStore) then
    GetProcedureAddress(Pointer(@_CertOpenStore), crypt32, 'CertOpenStore');
  if Assigned(_CertOpenStore) then
    Result:=_CertOpenStore(lpszStoreProvider,dwEncodingType,hCryptProv,dwFlags,pvPara)
  else
    Result:=nil;
end;
...
begin
  if not Assigned(_CertFreeCertificateContext) then
    GetProcedureAddress(Pointer(@_CertFreeCertificateContext), crypt32, 'CertFreeCertificateContext');
  if Assigned(_CertFreeCertificateContext) then
    Result:=_CertFreeCertificateContext(pCertContext)
  else
    Result:=False;
end;
...
begin
  GetProcedureAddress(_CertRDNValueToStr, crypt32, 'CertRDNValueToStr' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF});
  asm
        MOV     ESP, EBP
        POP     EBP
...
type
  TCertNameToStrA = function(dwCertEncodingType: DWORD; pName: PCERT_NAME_BLOB;
                             dwStrType: DWORD; psz: LPSTR; csz: DWORD): DWORD; stdcall;
var
  _CertNameToStrA: TCertNameToStrA = nil;

function CertNameToStrA;
begin
  if not Assigned(_CertNameToStrA) then
    GetProcedureAddress(Pointer(@_CertNameToStrA), crypt32,'CertNameToStrA');
  if Assigned(_CertNameToStrA) then
    Result:=_CertNameToStrA(dwCertEncodingType,pName,dwStrType,psz,csz)
  else
    Result:=0;
end;

type
  TCertNameToStrW = function(dwCertEncodingType: DWORD; pName: PCERT_NAME_BLOB;
                             dwStrType: DWORD; psz: LPWSTR; csz: DWORD): DWORD; stdcall;
var
  _CertNameToStrW: TCertNameToStrW = nil;

function CertNameToStrW;
begin
  if not Assigned(_CertNameToStrW) then
    GetProcedureAddress(Pointer(@_CertNameToStrW), crypt32,'CertNameToStrW');
  if Assigned(_CertNameToStrW) then
    Result:=_CertNameToStrW(dwCertEncodingType,pName,dwStrType,psz,csz)
  else
    Result:=0;
end;
...
begin
  {$IFDEF UNICODE}
  Result:=CertNameToStrW(dwCertEncodingType,pName,dwStrType,psz,csz);
  {$ELSE}
  Result:=CertNameToStrA(dwCertEncodingType,pName,dwStrType,psz,csz);
  {$ENDIF}
end;
...
begin
  GetProcedureAddress(_CertStrToName, crypt32, 'CertStrToName' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF});
  asm
        MOV     ESP, EBP
        POP     EBP
        JMP     [_CertStrToName]
...
type
  TCertGetNameStringA = function (pCertContext: PCCERT_CONTEXT; dwType, dwFlags: DWORD;
                                  pvTypePara: Pointer; pszNameString: LPSTR; cchNameString: DWORD): DWORD; stdcall;
var
  _CertGetNameStringA: TCertGetNameStringA = nil;

function CertGetNameStringA;
begin
  if not Assigned(_CertGetNameStringA) then
    GetProcedureAddress(Pointer(@_CertGetNameStringA),crypt32,'CertGetNameStringA');
  if Assigned(_CertGetNameStringA) then
    Result:=_CertGetNameStringA(pCertContext,dwType,dwFlags,pvTypePara,pszNameString,cchNameString)
  else
    Result:=0;
end;

type
  TCertGetNameStringW = function (pCertContext: PCCERT_CONTEXT; dwType, dwFlags: DWORD;
  pvTypePara: Pointer; pszNameString: LPWSTR; cchNameString: DWORD): DWORD; stdcall;
var
...
begin
  if not Assigned(_CertGetNameStringW) then
    GetProcedureAddress(Pointer(@_CertGetNameStringW),crypt32,'CertGetNameStringW');
  if Assigned(_CertGetNameStringW) then
    Result:=_CertGetNameStringW(pCertContext,dwType,dwFlags,pvTypePara,pszNameString,cchNameString)
  else
    Result:=0;
end;
...
begin
  {$IFDEF UNICODE}
  Result:=CertGetNameStringW(pCertContext,dwType,dwFlags,pvTypePara,pszNameString,cchNameString);
  {$ELSE}
  Result:=CertGetNameStringA(pCertContext,dwType,dwFlags,pvTypePara,pszNameString,cchNameString);
  {$ENDIF}
end;
...
begin
  GetProcedureAddress(_CertOpenSystemStore, crypt32, 'CertOpenSystemStore' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF});
  asm
        MOV     ESP, EBP
        POP     EBP
...
begin
  GetProcedureAddress(_CertAddEncodedCertToSysStore, crypt32, 'CertAddEncodedCertificateToSystemStore' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF});
  asm
        MOV     ESP, EBP
        POP     EBP
...
type
  TCryptQueryObject = function(dwObjectType: DWORD; pvObject: Pointer;
                               dwExpectedContentTypeFlags, dwExpectedFormatTypeFlags, dwFlags: DWORD;
                               pdwMsgAndCertEncodingType, pdwContentType, pdwFormatType: LPDWORD;
                               phCertStore: PHCERTSTORE; phMsg: PHCRYPTMSG; ppvContext: PPointer): BOOL; stdcall;
var
  _CryptQueryObject: TCryptQueryObject = nil;

function CryptQueryObject;
begin
  if not Assigned(_CryptQueryObject) then
    GetProcedureAddress(Pointer(@_CryptQueryObject), crypt32, 'CryptQueryObject');
  if Assigned(_CryptQueryObject) then
    Result:=_CryptQueryObject(dwObjectType,pvObject,dwExpectedContentTypeFlags,dwExpectedFormatTypeFlags,dwFlags,
...
begin
  GetProcedureAddress(_CryptRetrieveObjectByUrl, cryptnet, 'CryptRetrieveObjectByUrl' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF});
  asm
        MOV     ESP, EBP
        POP     EBP
...
type
  TCryptProtectData = function(pDataIn: PDATA_BLOB; szDataDescr: LPCWSTR;
   pOptionalEntropy: PDATA_BLOB; pvReserved: PVOID;
   pPromptStruct: PCRYPTPROTECT_PROMPTSTRUCT; dwFlags: DWORD; pDataOut: PDATA_BLOB): BOOL; stdcall;
var
  _CryptProtectData: TCryptProtectData = nil;

function CryptProtectData;
begin
  if not Assigned(_CryptProtectData) then
    GetProcedureAddress(Pointer(@_CryptProtectData), crypt32, 'CryptProtectData');
  if Assigned(_CryptProtectData) then
    Result:=_CryptProtectData(pDataIn,szDataDescr,pOptionalEntropy,pvReserved,
...
type
  TCryptUnprotectData = function(pDataIn: PDATA_BLOB; ppszDataDescr: LPLPWSTR;
  pOptionalEntropy: PDATA_BLOB; pvReserved: PVOID;
  pPromptStruct: PCRYPTPROTECT_PROMPTSTRUCT; dwFlags: DWORD; pDataOut: PDATA_BLOB): BOOL; stdcall;
var
  _CryptUnprotectData: TCryptUnprotectData = nil;

function CryptUnprotectData;
begin
  if not Assigned(_CryptUnProtectData) then
    GetProcedureAddress(Pointer(@_CryptUnprotectData), crypt32, 'CryptUnprotectData');
  if Assigned(_CryptUnProtectData) then
    Result:=_CryptUnProtectData(pDataIn,ppszDataDescr,pOptionalEntropy,pvReserved,
                                pPromptStruct,dwFlags,pDataOut)
  else
    Result:=False;
end;
...
type
  TCryptBinaryToStringA = function(const pBinary: PBYTE; cbBinary: DWORD;
    dwFlags: DWORD; pszString: LPSTR; var pchString: DWORD): BOOL; stdcall;
var
  _CryptBinaryToStringA: TCryptBinaryToStringA = nil;

function CryptBinaryToStringA;
begin
  if not Assigned(_CryptBinaryToStringA) then
    GetProcedureAddress(Pointer(@_CryptBinaryToStringA), crypt32, 'CryptBinaryToStringA');
  if Assigned(_CryptBinaryToStringA) then
    Result:=_CryptBinaryToStringA(pBinary,cbBinary,dwFlags,pszString,pchString)
  else
    Result:=False;
end;

type
  TCryptBinaryToStringW = function(const pBinary: PBYTE; cbBinary: DWORD;
    dwFlags: DWORD; pszString: LPWSTR; var pchString: DWORD): BOOL; stdcall;
var
  _CryptBinaryToStringW: TCryptBinaryToStringW = nil;

function CryptBinaryToStringW;
begin
  if not Assigned(_CryptBinaryToStringW) then
    GetProcedureAddress(Pointer(@_CryptBinaryToStringW), crypt32, 'CryptBinaryToStringW');
  if Assigned(_CryptBinaryToStringW) then
    Result:=_CryptBinaryToStringW(pBinary,cbBinary,dwFlags,pszString,pchString)
  else
    Result:=False;
end;

type
  TCryptStringToBinaryA = function(pszString: LPCSTR; cchString: DWORD;
    dwFlags: DWORD; ppBinary: PBYTE; var ppcbBinary: DWORD;
    ppdwSkip,pdwFlags: PDWORD): BOOL; stdcall;
var
  _CryptStringToBinaryA: TCryptStringToBinaryA = nil;

function CryptStringToBinaryA;
begin
  if not Assigned(_CryptStringToBinaryA) then
    GetProcedureAddress(Pointer(@_CryptStringToBinaryA), crypt32, 'CryptStringToBinaryA');
  if Assigned(_CryptStringToBinaryA) then
    Result:=_CryptStringToBinaryA(pszString,cchString,dwFlags,ppBinary,ppcbBinary,ppdwSkip,pdwFlags)
  else
    Result:=False;
end;

type
  TCryptStringToBinaryW = function(pszString: LPCWSTR; cchString: DWORD;
    dwFlags: DWORD; ppBinary: PBYTE; var ppcbBinary: DWORD;
    ppdwSkip,pdwFlags: PDWORD): BOOL; stdcall;
var
  _CryptStringToBinaryW: TCryptStringToBinaryW = nil;

function CryptStringToBinaryW;
begin
  if not Assigned(_CryptStringToBinaryW) then
    GetProcedureAddress(Pointer(@_CryptStringToBinaryW), crypt32, 'CryptStringToBinaryW');
  if Assigned(_CryptStringToBinaryW) then
    Result:=_CryptStringToBinaryW(pszString,cchString,dwFlags,ppBinary,ppcbBinary,ppdwSkip,pdwFlags)
  else
    Result:=False;
end;
...
begin
  {$IFDEF UNICODE}
  Result:=CryptStringToBinaryW(pszString,cchString,dwFlags,ppBinary,ppcbBinary,ppdwSkip,pdwFlags);
  {$ELSE}
  Result:=CryptStringToBinaryA(pszString,cchString,dwFlags,ppBinary,ppcbBinary,ppdwSkip,pdwFlags);
  {$ENDIF}
end;
...
begin
  {$IFDEF UNICODE}
  Result:=CryptBinaryToStringW(pBinary,cbBinary,dwFlags,pszString,pchString)
  {$ELSE}
  Result:=CryptBinaryToStringA(pBinary,cbBinary,dwFlags,pszString,pchString)
  {$ENDIF}
end;

type
  TPFXImportCertStore = function(pPFX: PCRYPT_DATA_BLOB; szPassword: LPCWSTR; dwFlags: DWORD): HCERTSTORE; stdcall;
var
  _PFXImportCertStore: TPFXImportCertStore = nil;

function PFXImportCertStore(pPFX: PCRYPT_DATA_BLOB; szPassword: LPCWSTR; dwFlags: DWORD): HCERTSTORE; stdcall;
begin
  if not Assigned(_PFXImportCertStore) then
    GetProcedureAddress(Pointer(@_PFXImportCertStore), crypt32, 'PFXImportCertStore');
  if Assigned(_PFXImportCertStore) then
    Result:=_PFXImportCertStore(pPFX,szPassword,dwFlags)
  else
    Result:=nil;
end;
...
function CryptAcquireContext; external advapi32 name 'CryptAcquireContext' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF};
...
function CryptSignHash; external advapi32 name 'CryptSignHash' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF};
...
function CryptVerifySignature; external advapi32 name 'CryptVerifySignature' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF};
...
function CryptSetProvider; external advapi32 name 'CryptSetProvider' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF};
...
function CryptSetProviderEx; external advapi32 name 'CryptSetProviderEx' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF};
...
function CryptGetDefaultProvider; external advapi32 name 'CryptGetDefaultProvider' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF};
...
function CryptEnumProviderTypes; external advapi32 name 'CryptEnumProviderTypes' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF};
...
function CryptEnumProviders; external advapi32 name 'CryptEnumProviders' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF};
...
function CertRDNValueToStr; external crypt32 name 'CertRDNValueToStr' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF};
function CertNameToStrA; external crypt32 name 'CertNameToStrA';
function CertNameToStrW; external crypt32 name 'CertNameToStrW';
function CertNameToStr; external crypt32 name 'CertNameToStr' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF};
function CertStrToNameA; external crypt32 name 'CertStrToNameA';
function CertStrToNameW; external crypt32 name 'CertStrToNameW';
function CertStrToName; external crypt32 name 'CertStrToName' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF};
...
function CertGetNameString; external crypt32 name 'CertGetNameString' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF};
...
function CertOpenSystemStore; external crypt32 name 'CertOpenSystemStore' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF};
...
function CertAddEncodedCertificateToSystemStore; external crypt32 name 'CertAddEncodedCertificateToSystemStore' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF};
...
function CryptRetrieveObjectByUrl; external cryptnet name 'CryptRetrieveObjectByUrl' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF};
...
function CryptBinaryToString; external crypt32 name 'CryptBinaryToString' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF};
...
function CryptStringToBinary; external crypt32 name 'CryptToStringBinary' + {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF};

{$ENDIF DYNAMIC_LINK}

function TrimAll;
begin
  Result:=StringReplace(ASource,#32,'',[rfReplaceAll,rfIgnoreCase]);
  Result:=StringReplace(Result,#160,'',[rfReplaceAll,rfIgnoreCase]);
...
function FormatToBinary(ASource: string; var ASize: Cardinal; AFormat: Cardinal = CRYPT_STRING_HEX): PByte;
begin
  CryptStringToBinary(PChar(ASource),Length(ASource),AFormat,nil,ASize,nil,nil);
  ReAllocMem(Result,ASize);
  CryptStringToBinary(PChar(ASource),Length(ASource),AFormat,Result,ASize,nil,nil);
end;
...
begin
  Result:='';
  CryptBinaryToString(ASource,ASize,AFormat,nil,sz);
  try
    SetLength(Result,sz);
    if CryptBinaryToString(ASource,ASize,AFormat,@Result[1],sz) then begin
      SetLength(Result,sz);
...
begin
  SetLength(Result,0);
  s:={$IFDEF UNICODE}WideToAnsi{$ENDIF}(AData);
  if (AHashType=CALG_SHA_256) or
     (AHashType=CALG_SHA_384) or
     (AHashType=CALG_SHA_512) then
    p:=PROV_RSA_AES
  else
    p:=PROV_RSA_FULL;
  try
    if not CryptAcquireContext(hProv,nil,nil,p,CRYPT_VERIFYCONTEXT) then
      Exit;
    if not CryptCreateHash(hProv,AHashType,0,0,hHash) then
      Exit;
    if not CryptHashData(hHash,@s[1],Length(s),0) then
      Exit;
    sz:=0;
    CryptGetHashParam(hHash,HP_HASHVAL,nil,sz,0);
    SetLength(Buffer,sz);
    if CryptGetHashParam(hHash,HP_HASHVAL,@Buffer[0],sz,0) then
      Result:=Buffer;
  finally
    Finalize(Buffer);
    if hHash>0 then
...
begin
  ...
  try
    ...
    try
      t:=AStream.Size;
      repeat
        r:=AStream.read(Buffer^,ABufferSize);
        if r>0 then begin
          if not CryptHashData(hHash,Buffer,r,0) then
            break;
        end;
        t:=t-r;
      until t<1;
      ...
      CryptGetHashParam(hHash,HP_HASHVAL,nil,sz,0);
      SetLength(Result,sz);
      CryptGetHashParam(hHash,HP_HASHVAL,@Result[0],sz,0);
    finally
      FreeMem(Buffer);
    end;
  finally
...
begin
  ...
  try
    ...
    buf:=ABuffer;
    r:=Min(ASize,ABufferSize);
    if r=0 then
      Exit;
    t:=0;
    repeat
      if not CryptHashData(hHash,buf,r,0) then
        break;
      inc(buf,r);
      inc(t,r);
      if ASize-t<r then
...
begin
  ...
  try
    if Assigned(mf.Content) then
      Result:=CreateBufferHash(mf.Content,mf.Size,AHashType,ABufferSize);
  finally
    mf.Free;
  end;
end;
...
begin
  ...
  try
    s:={$IFDEF UNICODE}WideToAnsi{$ENDIF}(ASource);
    ss.Write(s[1],Length(s));
    CryptStream(ss,sd,APassword,True,AHashType,ACipherType);
    SetLength(Result,sd.Size);
    sd.Read(Result[0],sd.Size);
  finally
    ss.Free;
    sd.Free;
  end;
...
function CryptBytes(const ASource: TBytes; APassword: string; AToCrypt: Boolean; AHashType: Cardinal = CALG_SHA1; ACipherType: Cardinal = CALG_RC4): TBytes;
var
  ss,sd: TMemoryStream;
begin
  SetLength(Result,0);
  ...
  sd:=TMemoryStream.Create;
  try
    ss.Write(ASource[0],Length(ASource));
    CryptStream(ss,sd,APassword,AToCrypt,AHashType,ACipherType);
    SetLength(Result,sd.Size);
    sd.Read(Result[0],sd.Size);
  finally
    ss.Free;
...
procedure CryptStream(const ASource, ADest: TStream; APassword: string; AToCrypt: Boolean; AHashType: Cardinal = CALG_SHA1; ACipherType: Cardinal = CALG_RC4);
var
  hProv: HCRYPTPROV;
  hHash: HCRYPTHASH;
  hKey: HCRYPTKEY;
...
begin
  ...
  ADest.Position:=0;
  try
    s:={$IFDEF UNICODE}WideToAnsi{$ENDIF}(APassword);
    ...
    try
      ...
      if s='' then begin
        ...
        if not CryptHashData(hHash,@s[1],Length(s),0) then
          Exit;
      if not CryptDeriveKey(hProv,ACipherType,hHash,0,hKey) then
        Exit;
    finally
      if hHash>0 then
        CryptDestroyHash(hHash);
    ...
    try
      repeat
        if eof then
          Break;
        len:=ASource.Read(Buffer^,bl);
        eof:=(ASource.Position>=ASource.Size);
        if AToCrypt then
          ok:=CryptEncrypt(hKey,0,eof,0,Buffer,len,len)
        else
          ok:=CryptDecrypt(hKey,0,eof,0,Buffer,len);
        if ok then
  ...
  try
    ...
    try
      repeat
        if eof then
          Break;
        Move(PByte(integer(ABuffer)+c)^,Buffer^,r);
        inc(c,r);
        len:=r;
        eof:=(c>=ASize);
        if AToCrypt then
...
begin
  try
    ...
    fsIn:=TFileStream.Create(ASource,fmOpenRead or fmShareDenyWrite);
    fsOut:=TFileStream.Create(ADest,fmCreate);
    try

      GetMem(Buffer, 512);
      try
        repeat
          eof:=(fsIn.Position>=fsIn.Size);
          if eof then
            Break;
...
var
  ...
  hHash: HCRYPTHASH;
  hKey: HCRYPTKEY;
  len,bl,kl,m,n,p: Cardinal;
  blob128: TAES_128_BLOB;
  blob192: TAES_192_BLOB;
  blob256: TAES_256_BLOB;
  iv: TAES_IV;
  s: ansistring;
  Buffer: PByte;
  ok,eof: Boolean;
begin
  ...
  try
    if not CryptAcquireContext(hProv,nil,nil,PROV_RSA_AES,0) then
      if not CryptAcquireContext(hProv,nil,nil,PROV_RSA_AES,CRYPT_NEWKEYSET) then
        Exit;

    s:={$IFDEF UNICODE}WideToAnsi{$ENDIF}(APassword);
    if AHAshType>0 then begin
      try
        if not CryptCreateHash(hProv,AHashType,0,0,hHash) then
          Exit;
        if s='' then begin
      ...
      FillChar(blob256.Key,SizeOf(blob256.Key),0);
      FillChar(blob192.Key,SizeOf(blob192.Key),0);
      FillChar(blob128.Key,SizeOf(blob128.Key),0);

      case ACipherType of
        CALG_AES_256: begin
          blob256.Header.bType:=PLAINTEXTKEYBLOB;
          blob256.Header.bVersion:=CUR_BLOB_VERSION;
          blob256.Header.Reserved:=0;
          blob256.Header.aiKeyAlg:=ACipherType;
          blob256.KeySize:=SizeOf(blob256.Key);
          if s<>'' then begin
            s:=Copy(s,1,SizeOf(blob256.Key));
            Move(PAnsiChar(s)[0],blob256.Key,Length(s));
          end;
          if not CryptImportKey(hProv,@blob256,SizeOf(blob256.Header)+SizeOf(blob256.KeySize)+SizeOf(blob256.Key),0,0,hKey) then
            Exit;
        end;
        CALG_AES_192: begin
          blob192.Header.bType:=PLAINTEXTKEYBLOB;
          blob192.Header.bVersion:=CUR_BLOB_VERSION;
          blob192.Header.Reserved:=0;
          blob192.Header.aiKeyAlg:=ACipherType;
          blob192.KeySize:=SizeOf(blob192.Key);
          if s<>'' then begin
            s:=Copy(s,1,SizeOf(blob192.Key));
            Move(PAnsiChar(s)[0],blob192.Key,Length(s));
          end;
          if not CryptImportKey(hProv,@blob192,SizeOf(blob192.Header)+SizeOf(blob192.KeySize)+SizeOf(blob192.Key),0,0,hKey) then
            Exit;
        end;
        else begin
          blob128.Header.bType:=PLAINTEXTKEYBLOB;
          blob128.Header.bVersion:=CUR_BLOB_VERSION;
          blob128.Header.Reserved:=0;
          blob128.Header.aiKeyAlg:=ACipherType;
          blob128.KeySize:=SizeOf(blob128.Key);
          if s<>'' then begin
            s:=Copy(s,1,SizeOf(blob128.Key));
            Move(PAnsiChar(s)[0],blob128.Key,Length(s));
          end;
          if not CryptImportKey(hProv,@blob128,SizeOf(blob128.Header)+SizeOf(blob128.KeySize)+SizeOf(blob128.Key),0,0,hKey) then
            Exit;
        end;
      end;
    end;
    ...
    m:=ACipherMode;
    if not CryptSetKeyParam(hKey,KP_MODE,@m,0) then
      Exit;

    p:=PKCS5_PADDING;
    if not CryptSetKeyParam(hKey,KP_PADDING,@p,0) then
      Exit;

    FillChar(iv,SizeOf(iv),0);
    if AInitVector<>'' then begin
      s:={$IFDEF UNICODE}WideToAnsi{$ENDIF}(AInitVector);
      s:=Copy(s,1,SizeOf(iv));
      Move(PAnsiChar(s)[0],iv,SizeOf(iv));
    end;
    if not CryptSetKeyParam(hKey,KP_IV,@iv,0) then
      Exit;

    n:=SizeOf(kl);
    if not CryptGetKeyParam(hKey,KP_KEYLEN,@kl,n,0) then
      Exit;
    kl:=kl div 8;
    if not CryptGetKeyParam(hKey,KP_BLOCKLEN,@bl,n,0) then
      Exit;
    bl:=bl div 8;
    ...
    try
      repeat
        if eof then
          Break;
        ZeroMemory(Buffer,bl);
        len:=ASource.Read(Buffer^,bl);
        eof:=(ASource.Position>=ASource.Size);
        n:=len;
        if AEncrypt then begin
          CryptEncrypt(hKey,0,eof,0,nil,n,n);
          ok:=CryptEncrypt(hKey,0,eof,0,Buffer,len,n)
        end else
          ok:=CryptDecrypt(hKey,0,eof,0,Buffer,n);
        if ok then
          ADest.Write(Buffer^,n);
      until eof;
    finally
      FreeMem(Buffer,bl*2);
    end;
...
begin
  ...
  try
    ss.Write(ASource[0],Length(ASource));
    CryptAESStream(ss,sd,APassword,AInitVector,AEncrypt,ACipherMode,AHashType,ACipherType);
    SetLength(Result,sd.Size);
    sd.Read(Result[0],sd.Size);
  finally
    ss.Free;
...
begin
  ...
  try
    s:={$IFDEF UNICODE}WideToAnsi{$ENDIF}(ASource);
    ss.Write(s[1],Length(s));
    CryptAESStream(ss,sd,APassword,AInitVector,True,ACipherMode,AHashType,ACipherType);
    SetLength(Result,sd.Size);
    sd.Read(Result[0],sd.Size);
  finally
    ss.Free;
...
var
  ...
  n: DWORD;
begin
  SetLength(APrivate,0);
  SetLength(APublic,0);
  Result:=False;
  if CryptAcquireContext(hProv,nil,nil,PROV_RSA_FULL,CRYPT_VERIFYCONTEXT) then begin
    try
      if CryptGenKey(hProv,AT_KEYEXCHANGE,RSA1024BIT_KEY or CRYPT_EXPORTABLE,hKeyPair) then begin
        try
          if CryptExportKey(hKeyPair,0,PRIVATEKEYBLOB,0,nil,n) then begin
            SetLength(APrivate,n);
            if CryptExportKey(hKeyPair,0,PRIVATEKEYBLOB,0,@APrivate[0],n) then begin
              if CryptExportKey(hKeyPair,0,PUBLICKEYBLOB,0,nil,n) then begin
                SetLength(APublic,n);
                Result:=CryptExportKey(hKeyPair,0,PUBLICKEYBLOB,0,@APublic[0],n);
...
begin
  ...
  if not CryptAcquireContext(hProv,nil,nil,PROV_RSA_FULL,CRYPT_VERIFYCONTEXT) then
    Exit;
  try
    if not CryptImportKey(hProv,@AKey[0],Length(AKey),0,0,hKey) then
      Exit;
    n:=SizeOf(kl);
    if not CryptGetKeyParam(hKey,KP_KEYLEN,@kl,n,0) then
      Exit;
...
begin
  ...
  try
    ss.Write(ASource[0],Length(ASource));
    CryptStream(ss,sd,AKey,AToCrypt,ACipherType);
    SetLength(Result,sd.Size);
    sd.Read(Result[0],sd.Size);
  finally
    ss.Free;
...
begin
  ...
  for i:=0 to High(ASource) do
    Result:=Result+IntToHex(Ord(ASource[i]),2);
end;
