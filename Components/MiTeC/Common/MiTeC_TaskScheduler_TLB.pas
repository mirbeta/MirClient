...
function AddAutorunTask(const AName,AAuthor,ADesc,APath,AArgs,AUser: string; AHighestPrivs: boolean = False): boolean;
...
begin
  ...
  for i:=1 to RTC.Count do begin
    ...
    for j:=1 to TC.Count do
      if TC.Item[j].type_=TASK_TRIGGER_BOOT then
        r.Boot:=True
      else if TC.Item[j].type_=TASK_TRIGGER_LOGON then
        r.Logon:=True;
