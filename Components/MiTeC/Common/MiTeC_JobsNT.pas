...
type
  ...
  private
    ...
    function GetJob(Index: Integer): PNTJobRecord;
  ...
  public
    ...
    property Jobs[Index: Integer]: PNTJobRecord read GetJob;
  end;
...
function GetJobTime(ATime: Cardinal): TDateTime;
var
  jt,
  Hour,MSec,
  Min,Sec: Cardinal;
...
function TNTJobs.GetJob(Index: Integer): PNTJobRecord;
begin
  if Index<FJobs.Count then
    Result:=PNTJobRecord(FJobs.Objects[Index])
  else
    Result:=nil;
end;

function TNTJobs.GetJobCount: Cardinal;
begin
  Result:=FJobs.Count;
end;
