...
procedure TServerForm1.Timer1Timer(Sender: TObject);
  ...
  try
    ...
    if HS.FreeSmall>=HS2.FreeSmall then
      s:=s+IntToStr(HS.FreeSmall - HS2.FreeSmall)+' small'+#13#10
    else
      s:=s+'- '+IntToStr(HS2.FreeSmall - HS.FreeSmall)+' small'+#13#10;

    if HS.FreeBig>=HS2.FreeBig then
      s:=s+IntToStr(HS.FreeBig - HS2.FreeBig)+' big '+#13#10
    else
      s:=s+'- '+IntToStr(HS2.FreeBig - HS.FreeBig)+' big '+#13#10;

    if HS.TotalFree>=HS2.TotalFree then
      s:=s+IntToStr(HS.TotalFree - HS2.TotalFree)+' total'+#13#10
    else
      s:=s+'- '+IntToStr(HS2.TotalFree - HS.TotalFree)+' total'+#13#10;

    s:=s+'TOTAL MEM:'#13#10;

    if HS.TotalAddrSpace>=HS2.TotalAddrSpace then
      s:=s+IntToStr(HS.TotalAddrSpace - HS2.TotalAddrSpace)+' space'+#13#10
    else
      s:=s+'- '+IntToStr(HS2.TotalAddrSpace - HS.TotalAddrSpace)+' space'+#13#10;

    if HS.TotalAllocated>=HS2.TotalAllocated then
      s:=s+IntToStr(HS.TotalAllocated - HS2.TotalAllocated)+' used'+#13#10
    else
      s:=s+'- '+IntToStr(HS2.TotalAllocated - HS.TotalAllocated)+' used'+#13#10;

    if (HS.TotalAllocated+HS.TotalFree)>=(HS2.TotalAllocated+HS2.TotalFree) then
      s:=s+IntToStr(HS.TotalAllocated + HS.TotalFree - (HS2.TotalAllocated + HS2.TotalFree) )+' total'
    else
      s:=s+'- '+IntToStr(HS2.TotalAllocated + HS2.TotalFree - (HS.TotalAllocated + HS.TotalFree) )+' total';
...
procedure TServerForm1.btnMemClick(Sender: TObject);
  begin
  HS2:=GetHeapStatus;
  end;
