...
uses
  ...
  FMX_Controls, FMX_Forms,
  FMX_Dialogs, FMX_ExtCtrls, FMX_Types,
  FMX_Edit, FMX_TabControl, FMX_Objects,
...
{$ENDIF}
  ...
  eReqCnt.Text:='50';
  xRTCTimeouts.isChecked:=True;
  xReqAutoRepeat.isChecked:=True;
...
procedure TForm1.RtcClientConnect(Sender: TRtcConnection);
  ...
  if not Sender.inMainThread then
    ...
    Label10.Text:='Connected to '+Sender.PeerAddr+':'+Sender.PeerPort;
    end;
  end;
...
procedure TForm1.UpdateTimerTimer(Sender: TObject);
  ...
  lblClients.Text:=IntToStr(RtcClient.TotalClientConnectionCount)+
                      ' ('+IntToStr(ocnt)+' - '+IntToStr(ccnt)+')';
  lblFlood.Text:=Format('%.0n',[fcnt*1.0]);
  end;
