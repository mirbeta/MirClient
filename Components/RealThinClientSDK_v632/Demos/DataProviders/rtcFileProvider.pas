...
uses
  ...
  rtcTypes, rtcLog, rtcFastStrings,
    ...
    with Sender do
      ...
      if MyFileName<>'' then
        ...
        if fsize>0 then
          ...
          Request.Info.asText['fname']:=StringReplace(MyFileName,'/','\',[rfReplaceAll]);
    ...
    if Request.DataSize>Request.DataIn then
      ...
      if Response['CONTENT-RANGE']='' then // no need to show this for partial downloads
        ...
        XLog('ERR! '+PeerAddr+' > '+Request.Host+
             ...
             Int2Str(GetTickCount-Request.Info.asLargeInt['$time'])+' ms ='+
  ...
  with TRtcDataServer(Sender) do
    if Request.Complete then
      Write('<html><body>Your IP: '+PeerAddr+'<br>'+
            ...
            'Memory in use: '+RtcString(FormatFloat('0.### MB', Get_AddressSpaceUsed / 1024))+'</body></html>');
  end;
