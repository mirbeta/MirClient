...
uses
  ...
  rtcTypes, rtcInfo,
  rtcConn, rtcDataCli, rtcDataSrv, rtcHttpCli,
  rtcDataRoute, rtcCliPool, rtcSyncObjs,
  rtcLog, rtcFastStrings;
...
type
  ...
  TRtcLoadBalancerSession = class(TObject)
    public
    ID:RtcString;
    Close:boolean;
    end;
  ...
  public
    ...
    function AddServer(const ServerAddr,ServerPort,RootURI:RtcString;
                       ...
                       SessionTimeout: TDateTime):integer;
  ...
  published
    ...
    { Use a Response Buffer to load complete Responses from Servers into memory before forwarding them to Clients?
      ...
      Setting "ResponseBuffer" to TRUE will increase memory requirements of the Load Balancer and it NOT recommended. }
...
function TRtcLoadBalancer.AddServer(const ServerAddr, ServerPort, RootURI: RtcString;
      MaxConnections, MaxSessions: integer; SessionTimeout: TDateTime):integer;
  var
    Con:TRtcClientConPool;
  begin
  ReqCS.Acquire;
...
function TRtcLoadBalancer.ServerReady(index: integer): boolean;
  ...
  try
    if (index>=0) and (index<Req.PoolCount) then
      ...
      Result:= ( (Con.MaxSessionCount=0) or (Con.ActiveSessions<Con.MaxSessionCount) ) and
...
function TRtcLoadBalancer.GetBalanceDebugInfo: RtcString;
  ...
  try
    ...
    for a:=0 to Req.PoolCount-1 do
      begin
      Con:=TRtcClientConPool(Req.Pool[a]);
      Result:=Result+'Server['+Int2Str(a)+'] '+Con.FServerAddr+':'+Con.FServerPort+Con.FRootURI+'/ = '+
...
class function TRtcLoadBalancer.GetLoadBalancer(Sender: TRtcDataClient): TRtcLoadBalancer;
  begin
  if assigned(Sender.Info.Obj['$owner']) then
    Result:=TRtcLoadBalancer(Sender.Info.Obj['$owner'])
  else
    Result:=nil;
  end;
...
class function TRtcLoadBalancer.GetVirtualPath(Sender: TRtcDataClient): RtcString;
  begin
  if assigned(Sender.Info.Obj['$owner']) then
    Result:=TRtcLoadBalancer(Sender.Info.Obj['$owner']).VirtualPath
  else
    Result:='';
  end;
