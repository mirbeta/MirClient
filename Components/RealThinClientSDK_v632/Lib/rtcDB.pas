...
type
  ...
  { @abstract(DataSet Change Recorder)
    Used by TRtcDataSetMonitor and TRtcMemDataSet to record DataSet changes.
...
{$ENDIF}
  ...
  published
    { Set Active to TRUE to enable access to internal TRtcDataSet data if it was assigned to "asDataSet" or "asObject",
      ...
      Set Active to FALSE to disable access to internal TRtcDataSet and write any changes to disk if "FileName" is set. }
    property Active;
...
// Copy data from a Delphi TDataSet into a TRtcArray or TRtcRecords (used for transport)
...
{ Internal function for skipping TGraphicField headers insidea a BlobStream.
  ...
  if assigned(Filter) then
    for fld:=0 to Filter.FieldCount-1 do
      ...
      if Filter.isNull[fname] then
        SqlWhere:=SqlWhere+'('+TableName+'."'+fname+'" is NULL)'
      else
        SqlWhere:=SqlWhere+'('+TableName+'."'+fname+'" '+CompareOperator+' :'+ParamPrefix+IntToStr(fld)+')';
      end;
  end;
      ...
      for fld:=0 to orow.FieldCount-1 do
        ...
        if (orow.isType[fname]<>rtc_ByteStream) and
          ...
          if orow.isNull[fname] then
            ...
            SqlWhere:=SqlWhere+'('+TableName+'."'+fname+'" = :w'+IntToStr(fld)+')';
          end;
        end;
      for fld:=0 to nrow.FieldCount-1 do
        begin
...
procedure TRtcMemDataSet.InternalOpen;
  ...
  if not assigned(FData) then
    if (FileName<>'') and File_Exists(FileName) then
      FData := TRtcDataSet.FromCode( RtcBytesToString(ZDecompress_Ex(Read_FileEx(FileName))) )
    else
...
procedure TRtcMemDataSet.InternalClose;
  ...
  if (FileName<>'') and assigned(FData) then
    if FData.FieldCount>0 then
      Write_FileEx(FileName, ZCompress_Ex( RtcStringToBytes(FData.toCode) ,zcDefault));
...
{  TFieldType = (ftUnknown, ftString, ftSmallint, ftInteger, ftWord,
  ...
  if assigned(FData) then
    for a:=0 to FData.FieldCount-1 do
      ...
      FieldDefs.Add(fname, ftype, fsize, FData.FieldRequired[fname]);
      end;
  end;
    ...
    {$ENDIF}
      ...
      if Buff<>nil then
        ...
        case TRtcFieldTypes(Field.DataType) of
          ...
          ft_LongWord:
            LongWord(Buff^):=from_b.RowData.asCardinal[idx];
          ft_Boolean:
...
*)
        ...
        ft_LongWord:
          // TLongWordField
          to_b.RowData.asCardinal[idx]:=LongWord(Buff^);
        ft_Single:
          // TSingleField
