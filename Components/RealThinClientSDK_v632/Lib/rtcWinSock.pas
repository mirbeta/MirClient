...
{$IFNDEF WINDOWS}
  {$MESSAGE WARN 'rtcWinSock unit is ONLY for MS Windows.'}
{$ENDIF}
...
function WSocket_SetVarSin(var Sin: TSockAddr; const IP, Port: RtcString; Family, SockProtocol, SockType: integer; PreferIP4: Boolean): integer;
...
function WSocket_SetVarSin(var Sin: TSockAddr; const IP, Port: RtcString; Family, SockProtocol, SockType: integer; PreferIP4: Boolean): integer;
  ...
  function Get_Addr(var Hints: TAddrInfo; const Sin: PSockAddr): integer;
    var
      Addr: PAddrInfo;
    begin
    Addr := nil;
    try
      ...
      if Result = 0 then
        if Addr = nil then
          raise ERtcSocketError.Create('WSocket_SetVarSin: _GetAddrInfo() returned with Addr = nil.')
        else if Addr^.ai_addr = nil then
          raise ERtcSocketError.Create('WSocket_SetVarSin: _GetAddrInfo() returned with Addr.ai_addr = nil.')
        else
  ...
  if not IsNewApi(family) then
    ...
    r := Get_Addr(Hints1, @Sin1);
    Result := r;
    if r = 0 then
      sin := sin1
    else if TwoPass then
      begin
      r := Get_Addr(Hints2, @Sin2);
      Result := r;
      if r = 0 then
        sin := sin2
      else
