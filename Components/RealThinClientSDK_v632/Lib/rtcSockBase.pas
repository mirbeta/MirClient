...
type
  ...
  TRtcSocketMessageJob=class(TRtcJob)
  public
    Sock:TRtcSocketBase;
    Msg:word;
  ...
  TRtcSocketMesDataJob=class(TRtcSocketMessageJob)
  public
    Data:Cardinal;
    procedure Kill; override;
    function Run(Thr:TRtcThread):boolean; override;
  ...
  public
    ...
    procedure Call_Message(Msg:word; Data:Cardinal; HighPriority:boolean=False);
    function Call_ThrMessage(Msg:word; Data:Cardinal; HighPriority:boolean=False):boolean;
...
procedure TRtcSocketBase.Call_Message(Msg: word; Data: Cardinal; HighPriority:boolean=False);
  var
  ...
  if not FEventsOff then
    ...
    if assigned(mt) then
      ...
      if (Data=0) and (Msg<=RTC_SOCKET_MAXMSG) then
        TRtcThread.PostJob(mt,Message_WsSock[Msg],HighPriority)
      else
        begin
        ...
        job.Msg:=Msg;
        job.Data:=Data;
        TRtcThread.PostJob(mt,job,HighPriority);
        end;
      end
    else
      DoMessage(Msg,Data);
...
function TRtcSocketBase.Call_ThrMessage(Msg: word; Data: Cardinal; HighPriority:boolean=False):boolean;
  var
    job:TRtcSocketMesDataJob;
    mt:TRtcThread;
  begin
...
{ TRtcSocketMessageJob }

procedure TRtcSocketMessageJob.Kill;
  begin
  Sock:=nil;
  // nothing
  end;

function TRtcSocketMessageJob.Run(Thr: TRtcThread):boolean;
  begin
  if assigned(Sock) then
    Sock.DoMessage(Msg,0);
  Result:=False;
  end;

{ TRtcSocketMesErrJob }

procedure TRtcSocketMesDataJob.Kill;
  begin
  inherited;
