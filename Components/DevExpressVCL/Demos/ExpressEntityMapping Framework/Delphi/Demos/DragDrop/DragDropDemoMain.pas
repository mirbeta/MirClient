...
{$ENDIF}
  ...
  cxLookAndFeelPainters, cxGridCustomLayoutView, dxSkinsCore, dxSkinDevExpressDarkStyle, dxSkinTheBezier, cxNavigator,
...
begin
  if not FilmsDemoDM.edsGenres.Active then
    FilmsDemoDM.edsGenres.Open;
  FilmsDemoDM.edsGenres.First;
  while not FilmsDemoDM.edsGenres.Eof do
  begin
    AMasterLevel := Grid.Levels.Add;
    AMasterLevel.MaxDetailHeight := 600;
    AMasterLevel.Caption := FilmsDemoDM.edsGenresNAME.AsString;
    ...
    AMasterLevel.Tag := FilmsDemoDM.edsGenresID.AsInteger;
...
begin
  ...
  try
    FilmsDemoDM.edsFilms.CheckBrowseMode;
    Grid.BeginUpdate;
    FilmsDemoDM.edsGenres.Locate('ID', ALevel.Tag, []);
...
begin
  ...
  if FilmsDemoDM.InsertFilm(FilmID, Grid.ActiveLevel.Tag) then
    ...
    MessageDlg('This film already exists in the category', mtInformation, [mbOk], 0);
end;
...
begin
  ...
  if FilmsDemoDM.InsertCompany(FilmID, CompanyID) then
    ...
    MessageDlg('This company is already assigned to the film', mtInformation, [mbOk], 0);
end;
...
begin
  ...
  if FilmsDemoDM.InsertPerson(FilmID, PersonID, 2) then
    ...
    MessageDlg('This person is already assigned to the film', mtInformation, [mbOk], 0);
end;
