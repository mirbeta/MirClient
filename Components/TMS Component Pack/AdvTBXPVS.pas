...
type
  ...
  {$EXTERNALSYM TFNDTTCallbackProc}
...
begin
  ...
  begin
    ...
    if DLLHandle > 0 then
    begin
      _DrawThemeTextEx := GetProcAddress(DLLHandle, 'DrawThemeTextEx'); // Do not localize
      if Assigned(_DrawThemeTextEx) then
...
begin
  ...
  try
    ...
    if DLLLoaded {and (ReferenceCount = 0)} then
    begin
